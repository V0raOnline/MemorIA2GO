...
def load_from_yaml(config_path: str | None = None,
    ...
    try:
        ...
        fallos = []
        if not exports_dir:
            fallos.append("'exports_dir' esta sin configurar")
        elif not exports_dir.is_dir():
            fallos.append(f"la carpeta de exports no existe: {exports_dir}")
        if not base_vault:
            fallos.append("'base_vault' esta sin configurar")
        if fallos:
        ...
        motivo = f"no pude leer la configuracion: {e}"
        warn(motivo)
        if problemas is not None:
...
def main():
    ...
    if not params:
        ...
        if args.no_wizard:
            error("No puedo empezar con esta configuracion:")
            for x in problemas or ["no encuentro rutas validas en el YAML"]:
                error(f"    {x}")
            error("Corrigelo en la pestana Configuracion, guarda, y vuelve a pulsar.")
            sys.exit(2)
        ...
        try:
            ...
            error("No puedo empezar con esta configuracion, y nadie contesta:")
            for x in problemas or ["no encuentro rutas validas en el YAML"]:
                error(f"    {x}")
            error("Corrigelo en la pestana Configuracion, guarda, y vuelve a pulsar.")
            sys.exit(2)
