...
T = {
    ...
    "carpeta_audio": "Audio",
    "carpeta_portadas": "Covers",
    "carpeta_conversaciones": "Conversations",
    "sin_conversacion": "_No conversation",
    "fichero_indice": "_index",
    "fichero_linaje": "_lineage",

    "sec_conversacion": "## Conversation",
    "sec_pistas_salidas": "## Tracks that came out of this",
    "hab_usuario": "You",
    "hab_agente": "Producer",
    "hab_herramienta": "uses",
    "conv_reintento": "retry",
    "conv_sin_mensajes": "No messages saved.",
    "conv_no_guardada": "No conversation saved. Run the backup again: "
                        "conversations have been saved since 2026-08-06, and "
                        "the missing ones fill themselves in.",

    "sec_familia": "## Family",
    "sec_instruccion": "## Instruction",
    "sec_letra": "## Lyrics",
    "sec_audio": "## Audio",

    "familia_original": "Original -- does not come from any earlier track.",
    "familia_de": "Comes from:",
    "familia_deriva": "These came out of it:",
    "instruccion_ninguna": "No instruction on record (the API only stores one "
                           "for edit operations).",
    "letra_ninguna": "No lyrics.",
    "wav_en_backup": "The lossless WAV is not copied into the vault; it is in "
                     "the backup as",
    "sin_audio": "No audio downloaded.",

    "idx_titulo": "# Flow Music",
    "idx_favoritas": "## ❤️ Liked",
    "idx_linaje": "## 🧬 Lineage",
    "idx_linaje_ver": "See [[_lineage]] -- a separate file, it is large.",
    "idx_conversaciones": "## Conversations",
    "idx_por_tipo": "## By operation type",
    "idx_resumen": "## Summary",

    "lin_titulo": "# Lineage",
    "lin_intro": "Each tree is one original track and everything that came out "
                 "of it. The Dewey code on each note gives its position: `0` is "
                 "the root, `0.1` its first descendant, `0.1.2` the second one "
                 "of that.",
    "lin_familias": "## Families with descendants",
    "lin_sueltas": "## Tracks with no descendants",
    "lin_pistas": "tracks",
    "lin_favoritas": "liked",
    ...
    "badges": {
        ...
        "audio__render_edit": "Edit",
        "audio__modify_song": "Modification",
        "audio__split_stems": "Stem",
        "audio_upload": "Upload",
        "audio__apply_effect": "Effect",
    },
    "badge_desconocido": "Other",
    "badge_favorita": "Liked",
}
...
def cargar_indice(backup_dir: Path) -> dict:
    ...
    if borradas:
        print(f"[info] {borradas} track(s) deleted in Flow Music, skipping")
    return vivas
...
def construir_indice(por_id: dict, nombres: dict, hijos: dict, raices: list) -> str:
    ...
    lineas.append("")
    lineas.append(f"- {total} tracks, {segundos / 3600:.1f} h")
    lineas.append(f"- {con_letra} with lyrics")
    lineas.append(f"- {con_linaje} with a declared source")
    lineas.append(f"- {len(raices)} family trees")
    lineas.append("")
...
def main():
    ...
    print("[info] loading the backup index...")
    por_id = cargar_indice(backup_dir)
    print(f"[info] {len(por_id)} tracks")

    print("[info] building the family tree (Dewey codes)...")
    codigos, hijos, raices = calcular_dewey(por_id)
    print(f"[info] {len(raices)} trees, "
          f"{sum(1 for h in hijos.values() if h)} tracks with descendants")
    ...
    if con_corchetes:
        print(f"[error] {len(con_corchetes)} note name(s) contain brackets; "
              f"their links will not resolve in Obsidian. Example: {con_corchetes[0]}")
        sys.exit(1)

    convs = cargar_conversaciones(backup_dir)
    nombres_conv = calcular_nombres_conversacion(convs)
    if convs:
        print(f"[info] {len(convs)} conversations loaded")
    else:
        print("[warn] no conversations saved in the backup -- "
              "run the backup again to fill them in")
    ...
    if huerfanos:
        print(f"[info] {huerfanos} orphan file(s) from earlier passes, removed")

    print(f"[info] track notes written: {len(por_id)}")
    if convs:
        print(f"[info] conversation notes written: {len(convs)}")
    if not args.no_copy_audio:
        print(f"[info] m4a copied: {audio_copiado} | covers: {portadas_copiadas}")
    if audio_ausente:
        print(f"[warn] {audio_ausente} track(s) with no m4a in the backup")
