...
def _recortar(s: str, n: int) -> str:
    ...
    if len(s) <= n:
        return s
    return s[:n] + "\n…[trimmed; full text in the session source]"
...
def nombres(sesion: dict) -> Dict[str, str]:
    ...
    return {
        "madre": base,
        "razonamientos": base + " · reasoning",
        "herramientas": base + " · tools",
    }
...
def _nota_madre(sesion: dict, nom: Dict[str, str], nivel: int,
    ...
    pares = [
        ...
        ("project", sesion.get("project")),
        ("branch", sesion.get("git_branch")),
    ]
    for k, v in (extra_madre or {}).items():
        pares.append((k, v))
    L = _fm(pares)
    L += ["# %s" % titulo(sesion), "",
          "> %s session. The conversation lives here; the reasoning and the "
          "tools are in sibling notes, linked turn by turn."
          % (sesion.get("provider") or "Agent"), ""]
    ...
    for t in sesion.get("turns", []):
        ...
        anclas = []
        if t.get("thinking"):
            anclas.append("[[%s#^t%d|thought]]" % (nom["razonamientos"], an))
...
def _nota_razonamientos(sesion: dict, nom: Dict[str, str]) -> str:
    L = _fm([("type", "reasoning"), ("from", "[[%s]]" % nom["madre"])])
    L += ["# Reasoning — %s" % titulo(sesion), "",
          "> The thinking blocks, turn by turn. Reached from the mother note. "
          "Only the turns where thinking was visible are here.",
          ""]
...
def _nota_herramientas(sesion: dict, nom: Dict[str, str], nivel: int) -> str:
    L = _fm([("type", "tools"), ("from", "[[%s]]" % nom["madre"])])
    etiqueta = {NIVEL_TRAZA: "one line per call",
                NIVEL_PLEGADO: "folded result",
                NIVEL_INTEGRO: "full result"}.get(nivel, "")
    L += ["# Tools — %s" % titulo(sesion), "",
          "> Each call with its result (%s). Reached from the mother note."
          % etiqueta, ""]
    for n, t in _turnos_asistente(sesion):
        if not t.get("tools"):
            continue
        ...
        for ll in t["tools"]:
            ...
            if nivel == NIVEL_TRAZA or not res:
                ...
                L += ["", "<details><summary>result</summary>", "",
