...
function renderSubstack(stats) {
  ...
  card.style.display = "";
  const retirados = s.retirados ? `${s.retirados} retired` : "";
  document.getElementById("dashboard-substack").innerHTML = [
    statBox("Posts", s.posts, s.export),
    statBox("Words", s.palabras.toLocaleString("en-GB")),
    statBox("Published", s.publicados, retirados),
    statBox("Drafts", s.borradores),
  ].join("");
}
...
function pintarVerificacion(d) {
  ...
  const filas = [
    sbRow("ok", SB_ICONO_ZIP, e.nombre,
      `${e.posts} posts · ${e.publicados} published · ${e.retirados} retired · ${e.borradores} drafts`),
  ];
  if (d.stats) {
    filas.push(sbRow("ok", SB_ICONO_CHART, d.stats.nombre,
      `matches ${d.stats.cruzan}/${d.stats.filas} · adds ${d.stats.secciones} sections and tags on ${d.stats.con_tags}`));
  } else {
    filas.push(sbRow("aviso", SB_ICONO_CHART, "No stats CSV",
      "notes will have no section, tags or metrics — download it from your Substack dashboard and drop it next to the export"));
  }
  filas.push(sbRow("aviso", SB_ICONO_ESCUDO, `${d.csv_de_terceros} CSV files with subscriber data`,
    "not your memory: never read"));
  ...
  if (a.comentarios !== undefined) {
    chips.push(`${a.comentarios} comments across ${a.posts_con_comentarios} posts`);
  }
  if (a.podcasts_sin_audio) chips.push(`audio for ${a.podcasts_sin_audio} podcast episodes`);
  if (a.imagenes) chips.push(`${a.imagenes} images, URL only`);

  let html = filas.join("");
  if (chips.length) {
    html += `<div class="sb-ausencias-label">What the export leaves out</div>
...
async function substackVerify() {
  ...
  btn.disabled = true;
  msg.textContent = "Looking inside the export...";
  msg.className = "msg";
  ...
  try {
    ...
    out.style.display = "";
    msg.textContent = "Export recognized.";
    msg.className = "msg ok";
  } catch (e) {
    msg.textContent = `Error: ${e.message}`;
    msg.className = "msg error";
...
async function substackBuild() {
  ...
  btn.disabled = true;
  msg.textContent = seco ? "Dry run..." : "Building the vault...";
  msg.className = "msg";
  out.style.display = "none";
  try {
    ...
    msg.textContent = seco ? "Dry run finished: nothing was written."
                           : "Vault built. Open it in Obsidian.";
    msg.className = "msg ok";
...
async function loadReconexion() {
  ...
  listEl.innerHTML =
    seccionPendientes("grok", "Grok", "your own Imagine generations", pendientesCache.grok) +
    seccionPendientes("chatgpt", "ChatGPT", "web search images", pendientesCache.chatgpt);
...
function seccionPendientes(proveedor, titulo, descripcion, items) {
  ...
  return `<details class="check-fold check-fold-warn" data-proveedor="${proveedor}" style="margin-bottom:8px; padding:12px 14px;">
    ...
    <div class="pendientes-cuerpo" style="margin-top:10px; padding-top:10px; border-top:1px solid var(--border);">
      <div class="empty-note">Loading list...</div>
    </div>
  </details>`;
}
...
function filaChatgpt(p) {
  const query = (p.queries || [])[0] || "(no search)";
  ...
  const contexto = convs.length > 1
    ? `${escapeHtml(convs[0])} · seen in ${convs.length} conversations`
    : escapeHtml(convs[0] || "");
  return `
    <div class="gizmo-row" data-pendiente-url="${escapeHtml(p.url || "")}">
      ...
      ${acciones(`<button class="btn secondary" data-pendiente-descartar>Discard</button>`)}
    </div>`;
}
...
function runPipeline(mode) {
  ...
  currentSource.onerror = () => {
    ...
    // estaba, no se pintaba nada. Pulsabas y no pasaba nada.
    if (!logBox.textContent.trim()) {
      appendLog("Could not start. There may be a run in progress: "
                + "wait for it to finish or restart M3M0R\u00b7IA.", "err");
    }
    setRunning(false);
    if (currentSource) currentSource.close();
  };
