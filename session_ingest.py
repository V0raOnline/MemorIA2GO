...
PROVEEDORES = {
    "claude-code": {"mod": cc, "banco": "CLAUDE_CODE/SESSIONS",
                    "sub": "Sessions-Code", "source": "claude_code_session"},
    "codex": {"mod": cx, "banco": "CODEX/SESSIONS",
              "sub": "Sessions-Codex", "source": "codex_session"},
}
...
def ingest_sesion(jsonl: Any, base_vault: Any, nivel: int = sn.NIVEL_PLEGADO
    ...
    madre_dir = merged / "Conversations" / cfg["sub"] / y / m
...
def ingest_dir(projects_dir: Any, base_vault: Any,
    ...
    log("%d session files under %s" % (len(jsonls), projects_dir))
    for j in jsonls:
        ...
        if (activa and _id_de(j) == activa) or _esta_creciendo(j):
            ...
            log("  SKIPPED (active session, partial capture): %s" % j.name[:40])
            continue
        try:
            escritas = ingest_sesion(j, base_vault, nivel=nivel)
        except Exception as e:
            log("  FAILED %s: %s" % (j.name, str(e)[:60]))
            continue
        if not escritas:
            stats["vacias"] += 1
            continue
...
def main() -> int:
    ...
    if args.base_vault:
        ...
        if not base_vault:
            print("base_vault not configured")
            return 2
    ...
    print()
    print("Sessions ingested: %d  ·  empty: %d  ·  notes written: %d"
          % (total["sesiones"], total["vacias"], total["notas"]))
    if total.get("activa_omitida"):
        print("Active session skipped: %d (re-run once you close it to "
              "capture it in full)" % total["activa_omitida"])
    print("Raw source preserved in MERGED_VAULT/{CLAUDE_CODE,CODEX}/SESSIONS")
    return 0
