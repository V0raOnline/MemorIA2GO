...
def _descargar_a_fichero(session, url, destino: Path, debug=False) -> bool:
    ...
    try:
        ...
        if esperado is not None:
            try:
                if parcial.stat().st_size != int(esperado):
                    print(f"  [aviso] descarga incompleta de {destino.name} "
                          f"({parcial.stat().st_size} de {esperado} bytes), se reintenta en la proxima pasada")
        ...
        print(f"  [error] escribiendo {destino.name}: {e}")
        parcial.unlink(missing_ok=True)
        return False
