...
se verifica es el reparto cerrado en CONTEXT 3y:
  ...
  - la madre cae en Conversations/Sessions-Code/AAAA/MM con el frontmatter que
...
def test_ingesta_completa_reparte_bien(tmp_path):
    ...
    # La madre en Conversations/Sessions-Code/AAAA/MM
    madre = escritas["madre"]
    assert "Conversations" in madre.parts and "Sessions-Code" in madre.parts
    assert "2026" in madre.parts and "08" in madre.parts

    # Las hijas en el banco, NO bajo Conversations
    for rol in ("razonamientos", "herramientas"):
        p = escritas[rol]
        assert "CLAUDE_CODE" in p.parts and "SESSIONS" in p.parts
        assert "Conversations" not in p.parts

    # La fuente cruda, integra, en el banco
    banco = merged / "CLAUDE_CODE" / "SESSIONS"
...
def test_la_madre_no_lleva_ruido_de_herramientas(tmp_path):
    ...
    tecnico: eso vive en la nota de herramientas, fuera de Conversations."""
    jsonl = _escribir_jsonl(tmp_path / "s.jsonl", _sesion_con_todo())
    escritas = si.ingest_sesion(jsonl, tmp_path / "vault", nivel=2)
...
def test_el_manifest_registra_la_sesion(tmp_path):
    ...
    manifest = json.loads(
        (tmp_path / "vault" / "MERGED_VAULT" / "CLAUDE_CODE" / "SESSIONS"
...
def test_idempotente_reingerir_no_duplica(tmp_path):
    ...
    banco = vault / "MERGED_VAULT" / "CLAUDE_CODE" / "SESSIONS"
    assert len(list(banco.glob("*.jsonl"))) == 1        # una sola fuente
    madres = list((vault / "MERGED_VAULT" / "Conversations").rglob("*.md"))
...
def test_codex_va_a_su_propio_banco_y_subcarpeta(tmp_path):
    """El proveedor se detecta por formato; Codex enruta a CODEX/SESSIONS y a
    Conversations/Sessions-Codex, no a los de Claude Code."""
    ...
    madre = escritas["madre"]
    assert "Sessions-Codex" in madre.parts
...
def test_los_dos_proveedores_conviven_en_una_carpeta(tmp_path):
    ...
    assert (merged / "CLAUDE_CODE" / "SESSIONS").exists()
    assert (merged / "CODEX" / "SESSIONS").exists()
