...
# ─────────────────────────────────────────
# Substack: reconocido y RECHAZADO a proposito.
# Guard escrito por agnt_substack; aqui llega con el mensaje en espanol,
# que es lo que le toca a esta edicion. Fixture sintetico inline, sin datos
# reales: el export de V0ra lleva emails de suscriptores y no tiene por que
# existir una copia en tests/.
# ─────────────────────────────────────────
...
def test_substack_zip_reconocido_y_rechazado(tmp_path):
    ...
    assert "EXPORT DE SUBSTACK" in result["mensaje"]
    ...
    assert "2 archivos CSV" in result["mensaje"]
